...
export default function ExportPanel({ cleanedBlob, originalName, userPlan, onExport, onUpgradePrompt }) {
  ...
  const estimatedSize = cleanedBlob ? Math.round(cleanedBlob.size / 1024 / 1024) : null;
  ...
  return (
    ...
    >
      <h2 className="font-display font-bold text-navy dark:text-white text-2xl">Export Your Video</h2>
      ...
      {estimatedSize && (
        <p className="text-xs text-slate-600 dark:text-slate-400">Estimated file size: ~{estimatedSize} MB</p>
      )}
      ...
      <motion.button
        ...
        aria-label={`Download ${FORMATS[format]?.label}`}
      >
        {exporting ? (
          <><Loader2 size={24} className="animate-spin" /> Preparing…</>
        ) : (
          <><Download size={24} /> Download {FORMATS[format]?.label}</>
        )}
      </motion.button>
    </motion.section>
  );
